...
from alcove.camera_worker import CameraWorker


def parse_args() -> Tuple[argparse.Namespace, list]:  # type: ignore
    """Parse command line arguments."""
    parser = argparse.ArgumentParser("Alcove")
    parser.add_argument(
        "--head-tracker",
    ...
    if not args.no_camera:
        ...
        if args.head_tracker is not None:
            if args.head_tracker == "yolo":
                from alcove.vision.yolo_head_tracker import HeadTracker
        ...
        if args.local_vision:
            try:
                from alcove.vision.processors import initialize_vision_manager
